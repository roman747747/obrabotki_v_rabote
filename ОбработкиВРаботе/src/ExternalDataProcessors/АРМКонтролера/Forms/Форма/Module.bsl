...
Процедура ПроведениеДокументовВФонеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Выполнено" Тогда
		...
		ДокументРеализацияТоваров = Ответ.ДокументРеализация; //tror20190819
		Попытка
...
//	//tror20181108 для Администратора не блокируем интерфейс
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
		Если ИмяСобытия = "ScanData" И МенеджерОборудованияКлиентПереопределяемый.ЕстьНеобработанноеСобытие() Тогда
			Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПеремещения ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПеремещениеЗаказаАРМ Тогда 
				ОбработатьШтрихкодыРаспоряжений(МенеджерОборудованияКлиент.ПреобразоватьДанныеСоСканераВСтруктуру(Параметр));
			ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПользователь  ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗавСклад Тогда
...
Процедура УстановитьВидимостьДоступность()
	...
	Элементы.КнопкаОк.Доступность = МожноЗакончитьКонтроль И НЕ МожноУпаковать;
	...
	//vvm 20181122( временно активируем кнопку продолжить на странице разхождения	
	Элементы.ОтложитьРассмотрение.Видимость = Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРасхождений И РазрешитьОткладыватьБлокировку;
	
	Элементы.КомандаПереместитьЗаказ.Видимость = Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаКнопкаПереместитьЗаказ;
	//vvm )
	...
	Элементы.КомандаЗаблокировать.Видимость = Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗавершить
							  ...
							  ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаАдминистрирования
							  ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗавСклад
							  ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПеремещениеЗаказаАРМ
							  ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаКнопкаПереместитьЗаказ
							  ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗаказПеремещен;
...
Процедура ОбработатьШтрихкодыПользователя(Данные)
	...
	Если ЗначениеЗаполнено(пТекущийПользователь) И Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗавСклад Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПеремещениеЗаказаАРМ;
		ТекущийПользователь = пТекущийПользователь;
		УстановитьВидимостьДоступность();
		Возврат;
	КонецЕсли;	
	
	Если ЗначениеЗаполнено(пТекущийПользователь) И Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПеремещениеЗаказаАРМ Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаКнопкаПереместитьЗаказ;
		ТекущийПользователь = пТекущийПользователь;
		УстановитьВидимостьДоступность();
		Возврат;
	КонецЕсли;	
	...
	Если ЗначениеЗаполнено(пТекущийПользователь) Тогда
		...
		ИначеЕсли ЗначениеЗаполнено(ТекущийЭтап) Тогда
			...
			КонецЕсли;
			ТекущийПользователь = пТекущийПользователь
		ИначеЕсли ЭтоЗавСкладом(пТекущийПользователь) Тогда
			ТекущийПользователь = пТекущийПользователь;
			Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗавСклад;
			Элементы.ДекорацияШККонтроллера.Видимость = Истина;
			Возврат;
		Иначе	
		...
		//vvm 20181122 (
		ПроверитьРазрешитьОткладыватьБлокировку(ТекущийПользователь);
		//vvm )
...
//vvm 20181122 (
Функция ЭтоЗавСкладом(пТекущийПользователь)
	...
	Запрос.УстановитьПараметр("Наименование", "ЗаведующийСкладом (Пользователи)");
	Запрос.УстановитьПараметр("Ссылка", пТекущийПользователь);
...
&НаСервере
Процедура ПроверитьРазрешитьОткладыватьБлокировку(ТекущийПользователь)
	...
	Запрос.УстановитьПараметр("Наименование", "РазрешитьОткладыватьБлокировку (Пользователи)");
	Запрос.УстановитьПараметр("Ссылка", ТекущийПользователь);
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если РезультатЗапроса.Пустой() Тогда
		РазрешитьОткладыватьБлокировку = Ложь;
	Иначе	 
		РазрешитьОткладыватьБлокировку = Истина;		
	КонецЕсли;	
...
Процедура ОбработатьШтрихкодыРаспоряжений(Данные)
	...
	Если МассивСсылок.Количество() > 0 Тогда
		...
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПеремещениеЗаказаАРМ Тогда		
			Если ПроверитьВозможностьПеремещенияЗаказа(пДокументОснование) Тогда
				Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаКнопкаПереместитьЗаказ;
			Иначе	
				ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Заказ нельзя переместить!'"),5, "Отказ");
			КонецЕсли;
		ИначеЕсли ПроверкаВозможностиРаботыСДокументом(пДокументОснование) Тогда
...
&НаСервере 
Функция ПроверитьВозможностьПеремещенияЗаказа(ЗначениеПроверки)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	врТекущийСтатусДокумента.Состояние = ЗНАЧЕНИЕ(Перечисление.КАБС_СостояниеДокументов.КВыполнению)
	               |	И врТекущийСтатусДокумента.Этап = ЗНАЧЕНИЕ(Перечисление.КАБС_ЭтапыБизнесПроцессов.Контроль)
	               |	И врТекущийСтатусДокумента.Пользователь = ЗНАЧЕНИЕ(Справочник.Пользователи.ПустаяСсылка)
	               |	И врТекущийСтатусДокумента.РабочееМесто <> &РабочееМесто";
...
Процедура КомандаСменитьПользователя(Команда)
	ТекущийПользователь = Неопределено;
	ДокументОснование   = Неопределено;
	ТекущийКонтроллер   = Неопределено;
	Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПользователь;
	втПараметрЗаказТовары.Очистить();
	УстановитьВидимостьДоступность();
КонецПроцедуры
...
Процедура КомандаПечать(Команда)
	...
	Если ЗначениеЗаполнено(ДокументПеремещение) Тогда 
		...
		Иначе
			...
			СтруктураДополнительныхПараметров.Вставить("Ссылка", ДокументПеремещение);
			...
			НапечататьДокумент(ДокументПеремещение, "ПеремещениеТоваровБС", СтруктураДополнительныхПараметров, "ФормаОбъекта");

		КонецЕсли;
		
		//tror20190819 печать реализации, если она создана по перемещению
		Если ЗначениеЗаполнено(ДокументРеализацияТоваров) Тогда //И ДокументРеализацияТоваров.Проведен Тогда
			СтруктураДополнительныхПараметров = Новый Структура;
			СтруктураДополнительныхПараметров.Вставить("Ссылка", ДокументРеализацияТоваров);
			СтруктураДополнительныхПараметров.Вставить("РежимОткрытияОкна", РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			СтруктураДополнительныхПараметров.Вставить("Редактирование", Ложь);
			СтруктураДополнительныхПараметров.Вставить("Защита", Истина);
			СтруктураДополнительныхПараметров.Вставить("СразуНаПринтер", Истина);
			...
			НапечататьДокумент(ДокументРеализацияТоваров, "РасходнаяНакладная", СтруктураДополнительныхПараметров, "ФормаОбъекта");
		КонецЕсли;
		//tror20190819>
...
Процедура НапечататьДокумент(ДокументСсылка, ИмяПечатнойФормы, СтруктураДополнительныхПараметров, ТипФормы = "ФормаОбъекта")
		...
		ИначеЕсли ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			ТипРаспечатываемогоДокумента = "Документ.РеализацияТоваровУслуг";
...
Процедура КомандаПродолжить(Команда)
	...
	Если ЗначениеЗаполнено(ДокументПеремещение) Тогда
		...
		ДокументРеализацияТоваров = Неопределено; //tror20190819
		ТекущийПользователь = Неопределено;
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПользователь;
	Иначе
...
//vvm 20181122 (
&НаКлиенте
Процедура ОтложитьРасхождение(Команда)					   
	ДокументПеремещение = Неопределено;
	ДокументРеализацияТоваров = Неопределено; //tror20190819
	Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПеремещения;
	УстановитьВидимостьДоступность();
...
&НаСервере
Функция КомандаПереместитьЗаказНаСервере()
	...
	Если Выборка.Следующий() Тогда
		НаборЗаписей = РегистрыСведений.КАБС_КонтрольРабочегоВремени.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.ДокументОснование.Установить(Выборка.ДокументОснование);
		НаборЗаписей.Отбор.ДокументОтгрузки.Установить(Документы.ПеремещениеТоваров.ПустаяСсылка());
		НаборЗаписей.Отбор.ДокументПриемки.Установить(Документы.ПриходныйОрдерНаТовары.ПустаяСсылка());
		НаборЗаписей.Отбор.УпаковочныйЛист.Установить(Документы.УпаковочныйЛист.ПустаяСсылка());

		НаборЗаписей.Прочитать();
		Для каждого ЗаписьНабора из НаборЗаписей Цикл
			Если ЗаписьНабора.Этап = Перечисления.КАБС_ЭтапыБизнесПроцессов.Контроль И 
					ЗаписьНабора.Состояние = Перечисления.КАБС_СостояниеДокументов.КВыполнению И 
					ЗаписьНабора.РабочееМесто <> НастройкиРМК.РабочееМесто И 
					ЗаписьНабора.Пользователь = Справочники.Пользователи.ПустаяСсылка() И
				    ЗаписьНабора.Склад = Выборка.Склад Тогда
	       		//установим текущее рабочее место
				ЗаписьНабора.РабочееМесто = НастройкиРМК.РабочееМесто;
				ЗаказПеремещен = Истина;
			КонецЕсли;
		КонецЦикла;
		 
		Если ЗаказПеремещен Тогда
...
&НаКлиенте
Процедура КомандаПереместитьЗаказ(Команда)
	Если ЗначениеЗаполнено(ДокументОснование) И КомандаПереместитьЗаказНаСервере() Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЗаказПеремещен;
		УстановитьВидимостьДоступность();
	Иначе	
		ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Заказ нельзя переместить'"),5, "Отказ");
	КонецЕсли;	
