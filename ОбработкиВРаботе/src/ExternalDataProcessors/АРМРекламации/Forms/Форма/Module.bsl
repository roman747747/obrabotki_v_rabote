...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Организация = Справочники.Организации.НайтиПоРеквизиту("ИНН","164604136394");
	...
	Если Строка(ПолучитьПараметрСеансаНаСервере("ТекущийПользователь")) = "Администратор" Тогда	
		...
		//для отладки, чтобы не сканировать шк пользователя, установим Текущего пользователя и страницу сразу
	...
	ДопСведениеПлантация = БС_ВызовСервера.ПолучитьЗначениеПараметраИзФайла("ДополнительныеКонстантыИБ", "ДополнительноеСведениеПлантация");
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(ТекущийПользователь) И Строка(ТекущийПользователь) = "Администратор" 
		И Элементы.Страницы.ТекущаяСтраница.Имя = "СтраницаСтарт" Тогда
	Иначе
...
#Область ОбработчикиСобытийЭлементовФормы
	...
	Процедура ПриходныеОрдераВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
		...
		Если ТипЗнч(ВыбраннаяСтрока) = Тип("ДокументСсылка.ПриходныйОрдерНаТовары") Тогда
			УстановитьТекущийПриходныйОрдерИТекущийЗаказПоставщику(ВыбраннаяСтрока);
		Иначе
			УстановитьТекущийПриходныйОрдерИТекущийЗаказПоставщику(Элемент.ТекущиеДанные.Ссылка);
		КонецЕсли;	
	...
	Для Каждого СтрокаТЧ ИЗ СписокТоваровДляПечатиЭтикеток Цикл
		...
		Если КолВоЭтикеток > 0 Тогда
			...
			Если КоличествоДляВыводаНаЭтикетке = 0   Тогда 
				НоваяСтрока.ШаблонЭтикетки = Справочники.ШаблоныЭтикетокИЦенников.НайтиПоНаименованию("Этикетка58х60 Срезка партии", Истина);
			Иначе
				НоваяСтрока.ШаблонЭтикетки = Справочники.ШаблоныЭтикетокИЦенников.НайтиПоНаименованию("Этикетка58х60 Срезка партии Рекламация штучный", Истина);
...
#Область ПолучениеДанных
	...
	Функция ПолучитьДанныеИзНастроек(НаименованиеНастройки)
		...
		Если НаименованиеНастройки = "СтатьяРасходов Недопоставка товара, выявленная в зоне рекламаций" Тогда
			...
			УстановитьПривилегированныйРежим(Истина);
	  		СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("MB-000022");
			УстановитьПривилегированныйРежим(Ложь);
	...
	&НаКлиенте
	Процедура ЗапроситьПлантациюУПользователя()
		
		Если ЗначениеЗаполнено(ТекущийЗаказПоставщику) Тогда
			ПараметрыОткрытия = Новый Структура;
			
			ПараметрыОткрытия.Вставить("Партнер", ЗначениеРеквизитаОбъектаПривилегированныйРежим(ТекущийЗаказПоставщику,"Партнер"));
			ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВводаПлантации", ЭтотОбъект, ПараметрыОткрытия);

			ОткрытьФорму("ВнешняяОбработка.АРМРекламации.Форма.ФормаВыбораПлантации", ПараметрыОткрытия, ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
		КонецЕсли;
						
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ПослеВводаПлантации(ВведенноеЗначение, ПараметрыОткрытия) Экспорт
		ТекущаяПлантация = ВведенноеЗначение;
	КонецПроцедуры	
	
	&НаСервере
	Функция ЗначениеРеквизитаОбъектаПривилегированныйРежим(ОбъектСсылка,ИмяРеквизита)
		
		ЗначениеРеквизита = Неопределено;
		УстановитьПривилегированныйРежим(Истина);
		ЗначениеРеквизита = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектСсылка, ИмяРеквизита);
		УстановитьПривилегированныйРежим(Ложь); 
	...
	&НаСервере
	Функция ПолучитьЗначениеДопСведениеОбъектаВПривилегированномРежиме(Ссылка, Свойство)
...
Процедура ПослеЗакрытияФормыСоздаваемогоПриходногоОрдера(Параметр1, Параметр2) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТекущаяПлантация) Тогда 
		ЗапроситьПлантациюУПользователя();
	КонецЕсли;	
...
&НаСервере
Процедура ЗаписатьДопСведениеОбъектаВПривилегированномРежиме(Объект, Свойство, Значение)
	...
	Попытка
		МЗ.Записать();
	Исключение
		ЗаписьЖурналаРегистрации("АРМ рекламации", УровеньЖурналаРегистрации.Ошибка, ,Объект, "Ошибка при записи дополнительного сведения плантация " +
...
Процедура ЗавершитьРаспределениеПриходаПоКачествуНаСервере()
	...
	Если ЗначениеЗаполнено(ВнутреннееПотреблениеТоваровПоПриходу) Тогда
		...
		ДокОбъект.Записать(РежимЗаписиДокумента.Проведение);
		ЗаписатьДопСведениеОбъектаВПривилегированномРежиме(ДокОбъект.Ссылка, ДопСведениеПлантация, ТекущаяПлантация);
	КонецЕсли;	
...
Процедура СоздатьИПровестиДокументыПоОтражениюРасхождений()
	...
	Если ДокОбъектПОТ.Товары.Количество() > 0 Тогда
		...
		Попытка
			...
			ЗаписатьДопСведениеОбъектаВПривилегированномРежиме(ДокОбъектСписание.Ссылка, ДопСведениеПлантация, ТекущаяПлантация);
		Исключение
...
Процедура УстановитьТекущийПриходныйОрдерИТекущийЗаказПоставщику(Ссылка, Отказ = Ложь)
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.ПеремещениеТоваров") Тогда	
		...
		Если ТЗРезультатЗапроса.Количество() > 0 Тогда
			...
			ТекущаяПлантация = ПолучитьЗначениеДопСведениеОбъектаВПривилегированномРежиме(Ссылка, ДопСведениеПлантация);
