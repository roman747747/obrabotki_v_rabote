...
Процедура ПечатьЭтикеток()
	...
	КоличествоДляВыводаНаЭтикетке = 0;
	Если Строка(СписокТоваров[0].Упаковка) = "шт." ИЛИ Строка(СписокТоваров[0].Упаковка) = "" И СписокТоваров[0].КоличествоУпаковок = 1 Тогда
		ВвестиЗначение(КоличествоДляВыводаНаЭтикетке, "Введите количество штук в этой пачке " +Символы.ПС + " для печати на этикетке");
	КонецЕсли;	 
	...
	Для Каждого СтрокаТЧ ИЗ СписокТоваров Цикл
		...
		Если КолВоЭтикеток > 0 Тогда
			...
			Иначе
				...
				НоваяСтрока.НаименованиеСкладаДляЭтикетки = "В этой пачке " + КоличествоДляВыводаНаЭтикетке + " шт."; 
			КонецЕсли;	
