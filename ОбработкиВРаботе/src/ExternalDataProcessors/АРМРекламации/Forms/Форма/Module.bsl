...
&НаКлиенте
Процедура КомандаПечатьУпаковочныйЛист(Команда)
	ТекущаяСтрока = Элементы.СписокПеремещение.ТекущаяСтрока;
	
	Если ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;       
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("ДокументСсылка", ТекущаяСтрока); 
	ОткрытьФорму("ВнешняяОбработка.АРМРекламации.Форма.ФормаПечатиУпаковочныйЛист", ПараметрыФормы);
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаСозданиеПеремещения(Команда)
	ДополнительныеПараметры = Новый Структура;
	ОписаниеОповещения = Новый ОписаниеОповещения("СоздатьПеремещение", ЭтотОбъект, ДополнительныеПараметры);
	ОткрытьФорму("ВнешняяОбработка.АРМРекламации.Форма.ФормаСозданияПеремещения", ДополнительныеПараметры, ЭтотОбъект,,,,ОписаниеОповещения,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс); 
КонецПроцедуры
...
&НаКлиенте
Процедура КомандаСоздатьБрак(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Склад", СкладОтправитель);
	ПараметрыОткрытия.Вставить("Организация", Организация);
	ОткрытьФорму("Документ.ВнутреннееПотреблениеТоваров.ФормаОбъекта",ПараметрыОткрытия,,,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаСоздатьПорчу(Команда)
	
	ПараметрыОткрытия = Новый Структура;
	ПараметрыОткрытия.Вставить("Склад", СкладОтправитель);
	ПараметрыОткрытия.Вставить("Организация", Организация);
	ОткрытьФорму("Документ.ПорчаТоваров.ФормаОбъекта",ПараметрыОткрытия,,,,,,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
&НаКлиенте
Процедура КомандаПереключитьРаскладку(Команда)
	//ПереключитьРаскладкуDW("Rus");
	//ПереключитьРаскладкуWS("Rus");
КонецПроцедуры
...
Процедура СоздатьДекорацию(Идентификатор, ИмяРеквизита, Заголовок, Значение, ГруппаРодитель, Доступность, Тип, Обработчик_ПриИзменении = Неопределено, Обработчик_АвтоПодбор = Неопределено, Ширина = Неопределено);
	...
	Иначе
		...
		Элемент.АвтоМаксимальнаяШирина = Ложь;
	КонецЕсли;
	Элемент.Высота = КолонкаВысота;
	//Элемент.КнопкаОткрытия = Ложь;
	
	Элемент.Шрифт = Новый Шрифт("Arial", 16);
	//Элемент.ШрифтЗаголовка = Новый Шрифт("Arial", 24);
	...
	//Если НЕ Обработчик_ПриИзменении = Неопределено Тогда
	//	Элемент.УстановитьДействие("ОбработкаНавигационнойСсылки", Обработчик_ПриИзменении);
	//КонецЕсли;
...
Функция ПолучитьСоздатьГруппу(Идентификатор, ИмяКнопки, Родитель, ВидГруппы)
	...
	//Элемент.Отображение = ОтображениеКнопки.КартинкаИТекст;
	//Элемент.Картинка = БиблиотекаКартинок.Папка;
КонецФункции
...
Процедура Подключаемая_ПриИзменении(Элемент)
	...
	Если СтрНайти(Элемент.Имя, "Реквизит") Тогда
		...
		ТекущиеДанные = СписокТоваров.Получить(Индекс);
...
Процедура Подключаемая_КомандаКнопки(Команда)
	...
	Строка = СписокТоваров.Получить(СтрЗаменить(Команда.Имя,"КомандаКнопки",""));
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПорча Тогда
		СформироватьПачкиНаСервере(Строка.Номенклатура, Строка.Характеристика);
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаРаспределитьПоКачеству Тогда
...
Процедура ОбработатьШтрихкодыПользователя(Данные)
	...
	Если ЗначениеЗаполнено(ТекущийПользователь) Тогда
		Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСтарт;
		//ВыдатьЗвуковоеСообщениеWindows(Истина);
		//ТекущийТовар = "";
	Иначе
...
&НаКлиенте
Процедура ПечатьШК(ПеремещениеСсылка)
	ТабличныеДокументы = Новый СписокЗначений;
	ОбъектыПечати = Новый СписокЗначений();
	
	ТабличныйДокумент = СформироватьУпаковочныйЛист(ПеремещениеСсылка);
	
	ТабличныйДокумент.ИмяПринтера = "Red";
	...
	ТабличныйДокумент.РазмерСтраницы = "Windows:256";
	ТабличныйДокумент.КоличествоЭкземпляров = 1;
	//ТабличныйДокумент.показать();
	ТабличныеДокументы.Добавить(ТабличныйДокумент, "ПечатьШкНаДокумент");
...
&НаСервере
Функция СформироватьУпаковочныйЛист(ДокументСсылка)
	...
	Область.Параметры.НомерДокумента = ДокументСсылка.Номер + Символы.ПС + Формат(ДокументСсылка.Дата, "ДЛФ=Д");
	ИнициализироватьРабочееМесто();
	Область.Параметры.ТекущийПользователь = ПользовательИБ;
...
Функция ПолучитьПараметры(ДокументДляПечати)
	...
	Для Каждого СтрокаТЧ ИЗ СписокТоваров Цикл
		...
		//Если ДокументДляПечати = "Хорошие" Тогда
		//	КолВоЭтикеток = СтрокаТЧ.КоличествоХороших;
		//Иначе
		//	КолВоЭтикеток = СтрокаТЧ.КоличествоПлохих;
		//КонецЕсли;	
		...
		Если КолВоЭтикеток > 0 Тогда
			...
			//КонецЕсли;	
			//Если ДеньНедели(ТекущаяДата())<=4 Тогда
			//	НомерПартии = "01";	
...
Функция ПолучитьНаименованиеСкладаДляЭтикетки(Склад)
	...
	Если ТЗРезультатЗапроса.Количество() > 0 Тогда
		...
		Иначе
			Возврат "Ошибка в определении значения доп. реквизита " + НаименованиеСвойстваНаименованиеСкладаДляЭтикетки;	
		КонецЕсли;	
	Иначе
		Возврат "ОШИБКА в определении значения доп. реквизита " + НаименованиеСвойстваНаименованиеСкладаДляЭтикетки;
	КонецЕсли;	
...
Процедура ПересчитатьКоличество()
	...
	Строка = СписокТоваров.Получить(Индекс);
...
&НаКлиенте
Процедура ВыдатьЗвуковоеСообщениеWindows(ВсеХорошо)
	
	Если НЕ ВсеХорошо Тогда
		//ПутьКФайлу = "C:\Windows\Media\Speech On.wav";
		//Иначе
		//ПутьКФайлу = "C:\Windows\Media\Speech Off.wav";
		ПутьКФайлу = "C:\Windows\Media\Error.wav";
...
&НаСервере
Процедура СоздатьПорчу(ТекущаяДата)
	
	УстановитьПривилегированныйРежим(Истина); //tror20190326
	
	НовыйДокумент = Документы.ПорчаТоваров.СоздатьДокумент();
	НовыйДокумент.Дата = ТекущаяДата;
	НовыйДокумент.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоНаименованию("Порча товара за счет поставщика");
	НовыйДокумент.Склад = СкладОтправитель;
	НовыйДокумент.АналитикаРасходов = ТекущийЗаказПоставщику;
	НовыйДокумент.ВидЦены = СкладОтправитель.УчетныйВидЦены;
	НовыйДокумент.Организация = Организация;
	НовыйДокумент.Ответственный = ТекущийПользователь;
	НовыйДокумент.ИсточникИнформацииОЦенахДляПечати = Перечисления.ИсточникиИнформацииОЦенахДляПечати.ПоВидуЦен;
	НовыйДокумент.Комментарий = "#АРМ Рекламации: Создан при разборке пачки";
	...
	Для Каждого Строка ИЗ СписокТоваров Цикл 
		...
		ИначеЕсли НЕ Строка.Количество = 0 Тогда
			Если Строка.Качество = Перечисления.ГрадацииКачества.ПустаяСсылка() Тогда
				СоздатьБрак(Строка, ТекущаяДата); 
	...
	Если НовыйДокумент.Товары.Количество() Тогда
...
&НаСервере
Процедура СоздатьБрак(Структура, ТекущаяДата)
	
	УстановитьПривилегированныйРежим(Истина); //tror20190326
	
	НовыйДокумент = Документы.ВнутреннееПотреблениеТоваров.СоздатьДокумент();
	НовыйДокумент.Дата = ТекущаяДата;
	НовыйДокумент.Склад = СкладОтправитель; 
	НовыйДокумент.Организация = Организация;
	НовыйДокумент.Ответственный = ТекущийПользователь;
	...
	НовыйДокумент.Комментарий = "#АРМ Рекламации: " + ?(ТекущаяОперация = "РаспределениеПоКачеству", "Создан при распредении прихода по качеству", "Создан при разборке пачки");
	
	НоваяСтрока = НовыйДокумент.Товары.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяСтрока, Структура, "Номенклатура, Характеристика, Количество, КоличествоУпаковок");
	НоваяСтрока.СтатьяРасходов = ПланыВидовХарактеристик.СтатьиРасходов.НайтиПоКоду("00-000005");
	НоваяСтрока.АналитикаРасходов = СкладОтправитель;
...
&НаСервере
Процедура СоздатьСборкаУпаковок_СтараяЛогика(ТекущаяДата)
	...
	НовыйДокумент.Дата = ТекущаяДата;
	Для Каждого Строка ИЗ СписокТоваров Цикл
		Если Строка.Количество И НЕ Строка.Качество = Перечисления.ГрадацииКачества.ПустаяСсылка() Тогда
			НоваяСтрока = НовыйДокумент.Товары.Добавить();
	...
	Элементы.СтраницыПорча.ТекущаяСтраница = Элементы.СтраницаСтартПорча;
	Элементы.СтраницыРаспределитьПоКачеству.ТекущаяСтраница = Элементы.СтраницаСтартРаспределитьПоКачеству;
