...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЕсли;	
	ЭтаФорма.Заголовок = "[АРМ рекламации v2.1] на рабочем месте " + Строка(ТекущееРабочееМесто) + "[ " + ТекущийРаздел + " ]"; 
...
Процедура КомандаСоздатьПеремещение(Команда)
	...
	//ДобавитьЭлементФормыВСписок(МассивНаименованийВидимыхЭлементов, "ТоварыВидЦены");
	//ДобавитьЭлементФормыВСписок(МассивНаименованийВидимыхЭлементов, "ТоварыЦена");
	//ДобавитьЭлементФормыВСписок(МассивНаименованийВидимыхЭлементов, "ТоварыСумма");
...
Процедура ПослеЗакрытияФормыСоздаваемогоПеремещенияНаСервере(ПеремещениеСсылка)
	...
	//tror20190423 СоздатьСборкаУпаковок(ТекущаяДата(), ПеремещениеСсылка); 
	//создание СборкиУпаковок по перемещению делается в расширении конфигурации при проведении документа
...
Процедура СформироватьКнопки()
	...
	//			   |	КАБС_ТоварыВУпаковкахНаСкладеОстатки.КоличествоОстаток КАК КоличествоУпаковок,
	//               |	1 КАК Коэффициент
	//               |ИЗ
	...
	//			   //|  И	
	//			   |	КАБС_ТоварыВУпаковкахНаСкладеОстатки.КоличествоОстаток > 0";
	//
	//			   
	Запрос.Текст = "ВЫБРАТЬ
				   |	ТоварыНаСкладахОстатки.Номенклатура КАК Номенклатура,
...
Процедура ОбработатьШтрихкодыПользователя(Данные)
	...
	Если ЗначениеЗаполнено(ПользовательпоШтрихкоду) Тогда
		Если РолиДоступны("КАБС_Рекламант", ПользовательпоШтрихкоду, Истина) Тогда
...
Процедура КомандаШапкаОтчет(Команда)
	...
	//ПараметрыФормы = Новый Структура("КлючВарианта", "Приемка в рекламации - для пользователя рекламант");
