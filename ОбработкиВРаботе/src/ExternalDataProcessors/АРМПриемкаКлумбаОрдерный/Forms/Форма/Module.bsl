...
#Область СобытияФормы
	...
	Процедура ПриОткрытии(Отказ)
		...
		Если Не ЗначениеЗаполнено(ИмяПечатнойФормыДляПриходногоОрдера) Тогда
	...
	Процедура ПриЗакрытии(ЗавершениеРаботы)
		...
		Если НЕ (Строка(ТекущийПользователь) = "Администратор")  ИЛИ РолиДоступны("Полные права", ТекущийПользователь, Истина)  Тогда
...
#Область КомандыФормы
	...
	//перезаполнение Списоков упаковочных листов для возврата, на случай прерывания работы
		...
		Если ЗначениеЗаполнено(ДокументПриемки) И СОКРЛП(ИмяПечатнойФормыДляПриходногоОрдера) <> "" Тогда
		...
		Если ЗначениеЗаполнено(ДокументАктОРасхождениях) ИЛИ Команда = "ПечатьАктаОРасхождениях" Тогда
			
			СтруктураОбъекта = Новый Структура;
			СтруктураОбъекта.Вставить("Ссылка", ДокументАктОРасхождениях);
			СтруктураОбъекта.Вставить("РежимОткрытияОкна", РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			СтруктураОбъекта.Вставить("Редактирование", Ложь);
			СтруктураОбъекта.Вставить("Защита", Истина);
			СтруктураОбъекта.Вставить("СразуНаПринтер", Истина);
			СтруктураОбъекта.Вставить("ОриентацияСтраницы", ОриентацияСтраницы.Ландшафт);
			СтруктураОбъекта.Вставить("АвтоМасштаб", Истина);
			
			Попытка
				ФормаАкта = ПолучитьФорму("Документ.АктОРасхожденияхПослеПеремещения.ФормаОбъекта"); 
				КомандаПечатиВПФ = ФормаАкта.Команды.Найти("ПодменюПечатьКомандаПечати0");
				УправлениеПечатьюКлиент.ВыполнитьПодключаемуюКомандуПечати(КомандаПечатиВПФ, ФормаАкта, СтруктураОбъекта);
			Исключение
				Сообщить("Неудачная попытка выполнить печать Акта о расхождениях (ВыполнитьПодключаемуюКомандуПечати)");
			КонецПопытки;
...
#Область ПолучениеДанных
	...
	функция ПолучитьТаблицуРасхожденияТоваров()
		...
		|ГДЕ
		|	НЕ втТоварыДокументаОтгрузки.Количество = КАБС_ПредварительныеСтрокиПеремещениеСрезПоследних.Количество";
...
Процедура НапечататьДокумент(ДокументСсылка, ИмяПечатнойФормы, СтруктураДополнительныхПараметров, ТипФормы = "ФормаОбъекта")
		...
		Попытка
			...
			Иначе
				...
				СообщениеПользователю = Новый СообщениеПользователю;
				СообщениеПользователю = "Не удачная попытка получить форму документа вида " + ВидРаспечатываемогоДокумента;
				СообщениеПользователю.Сообщить();
			...
			Иначе
				...
				СообщениеПользователю = Новый СообщениеПользователю;
				СообщениеПользователю = "Не найдена печатная форма для документа ""Приходный ордер на товары"" по имени " + ИмяПечатнойФормы;
				СообщениеПользователю.Сообщить();	
		...
		Исключение
			...
			СообщениеПользователю = Новый СообщениеПользователю;
			СообщениеПользователю = "Неудачная попытка выполнить печать " + ИмяПечатнойФормы + Символы.ПС + Строка(ИнформацияОбОшибке().Описание);
			СообщениеПользователю.Сообщить();
