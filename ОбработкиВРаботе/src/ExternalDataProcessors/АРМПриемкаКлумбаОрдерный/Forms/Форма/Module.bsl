		...
		Процедура ОбработатьШтрихкодыРаспоряжений(Данные)
			...
			Если МассивСсылок.Количество() > 0 Тогда
				...
				Иначе
					...
					СообщениеПользователюТекст = "По данному перемещению нет упаковочных листов к приемке от текущего водителя или вовзрату излишков и брака!";
					Если ЗначениеЗаполнено(СтруктураРезультатыПроверки) И СтруктураРезультатыПроверки.Свойство("СообщениеПользователюТекст", СообщениеПользователюТекст) Тогда 
						ПоказатьПредупреждение(Неопределено, СообщениеПользователюТекст, 5, "Отказ");
					Иначе	
						ПоказатьПредупреждение(Неопределено, "По данному перемещению нет упаковочных листов к приемке от текущего водителя или вовзрату излишков и брака!", 5, "Отказ");
					КонецЕсли;
		...
		Процедура ОбработатьШтрихкодыУЛ(Данные)
			...
			Если НЕ пУпаковочныйЛист = Неопределено Тогда
				...
				Если ПроверкаВозможностиРаботыСУЛ(пУпаковочныйЛист) Тогда
					...
					КонецЕсли;
				Иначе
					Сообщить("Этот упаковочный лист уже сосканирован и передан или не относится к обрабатываемому перемещению");
				КонецЕсли;
			Иначе
...
#Область ЗаписьДанных
	...
	#Область СозданиеДокументов
		...
		Функция СформироватьАктОРасхождениях(Отказ)
			...
			ДокументОбъект = НайтиИлиСоздатьАктОрасхождениях(); //Документы.АктОРасхожденияхПослеПеремещения.СоздатьДокумент();
