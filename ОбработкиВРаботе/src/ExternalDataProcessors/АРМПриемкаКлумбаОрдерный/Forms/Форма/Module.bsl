	...
	Процедура КомандаЗавершить(Команда)
		...
		Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПриемка Тогда
			...
			Для Каждого СтрокаДерева Из  ТоварыДерево.ПолучитьЭлементы() Цикл
				Если СтрокаДерева.КоличествоПлан > СтрокаДерева.КоличествоФакт + СтрокаДерева.КоличествоБрак Тогда
					СуммаРасхожденийПоУЛ = СуммаРасхожденийПоУЛ + (СтрокаДерева.КоличествоПлан - (СтрокаДерева.КоличествоФакт + СтрокаДерева.КоличествоБрак))*СтрокаДерева.Цена;
				ИначеЕсли СтрокаДерева.КоличествоПлан < СтрокаДерева.КоличествоФакт + СтрокаДерева.КоличествоБрак Тогда
					СуммаРасхожденийПоУЛ = СуммаРасхожденийПоУЛ + ((СтрокаДерева.КоличествоФакт + СтрокаДерева.КоличествоБрак) - СтрокаДерева.КоличествоПлан)*СтрокаДерева.Цена;	 
				КонецЕсли;	
			КонецЦикла;
			
			Если СуммаРасхожденийПоУЛ > 0 Тогда
				ТекстВопроса = "Вы уверены, что хотите завершить приемку по этому упаковочному листу с расхождениями на сумму " +Строка(СуммаРасхожденийПоУЛ) + "руб."+Символы.ПС + "Сумма расхождений может быть вычтена из зарплаты Вашей или менеджера отправителя ."; 
