			...
			ИначеЕсли ТекущийЭтап = "Возврат" Тогда
				...
				ИначеЕсли ТекущийПодэтап = "Передача водителю" Тогда
					...
					Если ВсеУЛПринятыВодителем() Тогда
						Предупреждение("Приемка и возврат излишков и непринятого товара завершены успешно!");
		...
		Процедура ЗавершитьПриемкуЗаказа(Отказ, СтруктураВозврата);
			...
			Если Результат[Результат.Количество() - 3].Пустой() Тогда //Все УЛ по перемещению приняты и проконтролированы, можно закрывать и переходить в этапу Возврат
				...
				Если (Не Результат1.Пустой()) И ЗначениеЗаполнено(ДокументОтгрузки) Тогда
					...
					ЕстьБрак = Результат[Результат.Количество()
						- 5].Выгрузить().Итог("КоличествоУпаковокБрак");
					ЕстьИзлишки = Результат[Результат.Количество()
						- 5].Выгрузить().Итог("КоличествоУпаковокИзлишек");
			...
			// только в этом случае создадим  письмо
				...
				Попытка
					...
					Исключение
						УспехОтправки = Ложь;
						Почта.Отключиться();
						ЗаписьЖурналаРегистрации("АРМПриемка.ОшибкаОтправкиПочты", УровеньЖурналаРегистрации.Ошибка, , , "Ошибка Отправки Письма ( ООП )"
							+ ОшибочныеПолучатели);
					КонецПопытки;
				...
				Исключение
					...
					ЗаписьЖурналаРегистрации("АРМПриемка.ОшибкаОтправкиПочты", УровеньЖурналаРегистрации.Ошибка, , , "Не получилось отправить письмо");
				КонецПопытки;
			Иначе
