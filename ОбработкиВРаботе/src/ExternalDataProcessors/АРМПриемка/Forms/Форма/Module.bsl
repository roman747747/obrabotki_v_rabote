//АРМПриемка - предназначениа для приемки товаров на ордерных торговых точках
#Область СобытияФормы
	&НаКлиенте
	Процедура ПриОткрытии(Отказ)
		МенеджерОборудованияКлиентПереопределяемый.НачатьПодключениеОборудованиеПриОткрытииФормы(ЭтаФорма, "СканерШтрихкода");
		//СанкционированоеЗакрытие = Ложь;
		...
		Если Источник = "ПодключаемоеОборудование" И ВводДоступен() Тогда
			Если ИмяСобытия = "ScanData"
				...
				ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПользователь
						...
						ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСтартВодитель Тогда
...
#Область КомандыФормы
				...
				ИначеЕсли ТекущийПодэтап = "Контроль" Тогда
					...
					Если Не Отказ Тогда
						...
						Если СтруктураВозврата.Свойство("ПриемкаЗавершена", ПриемкаЗавершена)
							...
							Предупреждение("Приемка завершена успешно");
	...
	//перезаполнение Списоков упаковочных листов для возврата, на случай прерывания работы
		...
		Если ЗначениеЗаполнено(ДокументПеремещениеВозврат) Тогда
			СтруктураОбъекта = Новый Структура;
			СтруктураОбъекта.Вставить("Ссылка", ДокументПеремещениеВозврат);

			МассивОбъектов = Новый Массив;
			МассивОбъектов.Добавить(ДокументПеремещениеВозврат);

			ПараметрыПечати = Новый Структура;
			ПараметрыПечати.Вставить("РежимОткрытияОкна", РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
			//УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.ПеремещениеТоваров", "НакладнаяНаПеремещение", МассивОбъектов, ЭтотОбъект, ПараметрыПечати);
			СтруктураОбъекта.Вставить("РежимОткрытияОкна", РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

			Попытка
				УправлениеПечатьюКлиент.ВыполнитьПодключаемуюКомандуПечати(Команды.Найти("ПодменюПечатьКомандаПечати13"), ПолучитьФорму("Документ.ПеремещениеТоваров.ФормаОбъекта"), СтруктураОбъекта);
			Исключение
				Сообщить("Неудачная попытка выполнить печать (ВыполнитьПодключаемуюКомандуПечати)");
			КонецПопытки;
		...
		//	ПараметрыПечати.Вставить("РежимОткрытияОкна",РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		//	МассивОбъектов = Новый Массив;
		//	МассивОбъектов.Добавить(ДокументАктОРасхождениях);
		//	УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.АктОРасхожденияхПрослеПеремещения", "УпаковочныйЛистСРасшифровкой", МассивОбъектов, ЭтотОбъект, ПараметрыПечати);
		//	ПечатьЭтикеток(УпаковочныйЛистВозвратИзлишков);
		//КонецЕсли;
	КонецПроцедуры
...
#Область ОбщиеПроцедурыИФункции
	...
	Процедура ИнициализироватьРабочееМесто(Отказ)
		...
		Если Выборка.Следующий() Тогда
			...
			//ИспользоватьУпаковочныеЛисты = НастройкиРМК.ИспользоватьУпаковочныеЛисты;
	...
	Процедура УстановитьВидимостьДоступность()
		...
		Элементы.КомандаЗаблокировать.Видимость = Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаДокумент
			ИЛИ Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПриемка
		...
		Элементы.КомандаУпаковать.Доступность = МожноУпаковать;
	...
	Функция ЗакрытьСистему()
		...
		Возврат НЕ (Строка(ПараметрыСеанса.ТекущийПользователь) = "Администратор");
...
#Область ШтрихкодыИТорговоеОборудование
	...
	#Область ОбработкаШтрихкодов
		...
		Процедура ОбработатьШтрихкодыПользователя(Данные)
			...
			Если ЗначениеЗаполнено(ПользовательПоШтрихкоду) Тогда
				...
				Иначе
					ПоказатьПредупреждение(Неопределено, НСтр("ru = 'Недостаточно прав!.'"));
				КонецЕсли
			Иначе
		...
		Процедура ОбработатьШтрихкодыРаспоряжений(Данные)
			...
			Если МассивСсылок.Количество() > 0 Тогда
				...
				Если ПроверкаВозможностиРаботыСДокументом(пДокументПеремещение) Тогда
					...
					УстановитьОтборСпискаУЛ();
				Иначе
					ПоказатьПредупреждение(Неопределено, НСтр("ru = 'По данному перемещению нет упаковочных листов к приемке от текущего водителя или вовзрату излишков и брака!'"), 5, "Отказ");
				КонецЕсли;
			Иначе
			//ШтрихкодированиеПечатныхФормКлиент.ОбъектНеНайден(Данные.Штрихкод);
				Сообщить("Не найден документ перемещение по шктрихкоду!");
			КонецЕсли;
		...
		Процедура ОбработатьШтрихкодыНоменклатурыНаСервере(ЭтоБрак,
			...
			Пока Выборка.Следующий() Цикл
				Если Выборка.КОформлениюОстаток = 0
						И Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПриемка Тогда
					Информация = "Отсутствует товар " + Выборка.Товар
						+ " в упаковочном листе!";
					Прервать;
				КонецЕсли;

				//проверка на остаток к отгрузке убрана для возможности приемки
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПриемка
							ИЛИ ВыборкаДетальныеЗаписи.КОформлениюОстаток >= ВыборкаДетальныеЗаписи.ОтгруженоОстаток
	...
	#Область ПроверкаВозможностиРаботы
		...
		Функция ПроверкаВозможностиРаботыСДокументом(ЗначениеПроверки)
			...
			Запрос.Текст = "ВЫБРАТЬ
				...
				|										И НЕ врСостояниеДокументов.Состояние = ЗНАЧЕНИЕ(Перечисление.КАБС_СостояниеДокументов.КВыполнению)
				...
				|ГДЕ
				|	втСостояниеДокументовНомераЭтапов.НомерЭтапа МЕЖДУ 3 И 7
				|
				|СГРУППИРОВАТЬ ПО
			...
			СписокСкладов.Добавить(СкладДляВозвратаБракаИИзлишков);
		...
		Функция ПроверкаВозможностиРаботыСУЛ(ЗначениеПроверки)
			...
			Запрос.Текст = "ВЫБРАТЬ
				...
				|ИЗ
				|	РегистрСведений.КАБС_КонтрольРабочегоВремени.СрезПоследних(, УпаковочныйЛист = &УпаковочныйЛист) КАК КАБС_КонтрольРабочегоВремениСрезПоследних
				|;
				|
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
	...
	Процедура ВызватьИнтерактивноеОкно(ДанныеШтрихкодов)
		...
		ОткрытьФорму("ВнешняяОбработка.АРМПриемка.Форма.ФормаВопрос", ПараметрыОткрытия, ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
...
#Область ПолучениеДанных
	...
	Процедура ЗаполнитьСпискиУЛВозврата()
		Если НЕ ЗначениеЗаполнено(СписокУпаковочныхЛистовИзлишков)
			...
			Результат = Запрос.ВыполнитьПакет();
			ТЗСписокУЛБрак = Результат[Результат.Количество() - 1].Выгрузить();
			ТЗСписокУЛИзлишки = Результат[Результат.Количество() - 2].Выгрузить();
	...
	функция ПолучитьТаблицуРасхожденияТоваров()
		...
		Запрос.Текст = "ВЫБРАТЬ
			...
			|	СУММА(КАБС_ПредварительныеСтрокиПеремещениеСрезПоследних.КодСтроки) КАК КодСтроки,
			...
			|	И (КАБС_ПредварительныеСтрокиПеремещениеСрезПоследних.ДокументОтгрузки = Значение(Документ.ПеремещениеТоваров.ПустаяСсылка)
			...
			|	КАБС_ПредварительныеСтрокиПеремещениеСрезПоследних.КоличествоУпаковок КАК КоличествоУпаковок,
			|	КАБС_ПредварительныеСтрокиПеремещениеСрезПоследних.Количество КАК Количество,
			|	КАБС_ПредварительныеСтрокиПеремещениеСрезПоследних.УпаковочныйЛист КАК УпаковочныйЛист,
			|	КАБС_ПредварительныеСтрокиПеремещениеСрезПоследних.УпаковочныйЛистРодитель КАК УпаковочныйЛистРодитель
			|ИЗ
	...
	&НаСервере
	Функция РолиДоступны(Пользователь, Роль, ПривилегированныйРежим = Ложь)
		Возврат Пользователи.РолиДоступны(Пользователь, Роль, ПривилегированныйРежим);
	КонецФункции
...
#Область ЗаписьДанных
	...
	#Область СозданиеДокументов
		...
		Функция СформироватьАктОРасхождениях(Отказ)
			...
			//ТоварыДляДокумента.Свернуть("Номенклатура, Характеристика, Упаковка, УпаковочныйЛист, Цена", "КоличествоУпаковокПоДокументу,КоличествоПоДокументу, КоличествоУпаковок, Количество, Сумма");
		...
		Процедура СоздатьДокументПриемки(ТаблицаЗначений, Период, Отказ = Ложь)
			...
			ТоварыДляДокумента = ТаблицаЗначений.Скопировать(, "ДокументОтгрузки, Номенклатура, Характеристика, Упаковка, УпаковочныйЛист, КоличествоУпаковок, Количество, Цена, Сумма");
			ТоварыДляДокумента.Свернуть("ДокументОтгрузки, Номенклатура, Характеристика, Упаковка, УпаковочныйЛист, Цена", "КоличествоУпаковок, Количество, Сумма");
			...
			ДокументОбъект.Распоряжение = ДокументОтгрузки;
			...
			//РТ 20180416 добавил для формирования движений приход на склад
	...
	#Область ЗавершениеЭтапов
		...
		Процедура ЗавершитьПриемкуЗаказа(Отказ, СтруктураВозврата);
			...
			Результат = Запрос.ВыполнитьПакет();
			Если НЕ Результат[Результат.Количество() - 2].Пустой() Тогда
				Сообщение = Новый СообщениеПользователю;
			...
			Если Результат[Результат.Количество() - 3].Пустой() Тогда //Все УЛ по перемещению приняты и проконтролированы, можно закрывать и переходить в этапу Возврат
				...
				Отказ = Ложь;
				Если (Не Результат1.Пустой()) И ЗначениеЗаполнено(ДокументОтгрузки) Тогда
					...
					Если Не Отказ Тогда
						...
						Если ЕстьИзлишки ИЛИ ЕстьБрак Тогда

							//запись Возврат начало
							НоваяЗапись = РегистрыСведений.КАБС_КонтрольРабочегоВремени.СоздатьМенеджерЗаписи();
							НоваяЗапись.Период = ТекущаяДатаСеанса();
							НоваяЗапись.ДокументОснование = ДокументОснование;
							...
							Если ЕстьИзлишки Тогда
								УстановитьТекущиеЭтапИПодэтап("Возврат", "Контроль излишков");
								ПерейтиНаСтраницуНаСервере(Элементы.СтраницаКонтрольИзлишков);
								ОбновитьДеревоТоваровПоТабличнымЧастям();
							Иначе
								ЗавершитьВозвратИзлишковИБрака(Отказ);
							КонецЕсли;
						...
						// Сообщаем пользователю о завершении приемки
							СтруктураВозврата.Вставить("ПриемкаЗавершена", Истина);
						КонецЕсли;
					Иначе
					// проблема при создании документа приемки
						Сообщить("Неудачная попытка создать документ приемки, обратитесь в техподдержку!");
					КонецЕсли;
				...
				Иначе
					...
					Отказ = Истина;
					Сообщение = Новый СообщениеПользователю;
					Сообщение.Текст = "Неудачная попытка создания документа приемки";
					Сообщение.Сообщить();
			...
			// есть непринятые УЛ по перемещению
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = "Есть непроконтролированные упаковочные листы, завершите их контроль";
				Сообщение.Сообщить();
				ПерейтиНаСтраницуНаСервере(Элементы.СтраницаУЛ);
...
#Область ПрочиеПроцедурыИФункции
	...
	Процедура ОбновитьДеревоТоваровПоТабличнымЧастям()
		...
		Запрос.Текст = "ВЫБРАТЬ
			...
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	ПеремещениеТоваровТовары.КодСтроки КАК НомерСтроки,
			|	ПеремещениеТоваровТовары.Номенклатура КАК Номенклатура,
	...
	Процедура ОбновитьДеревоТоваровПоИзлишкам()
		...
		Запрос.Текст = "ВЫБРАТЬ
			...
			|	втСтрокиДокументаОтгузки.Характеристика КАК Характеристика,
			|	втСтрокиДокументаОтгузки.Упаковка КАК Упаковка,
			|	втИзлишкиПоПеремещениюФакт.КоличествоФакт КАК КоличествоФакт,
			|	втИзлишкиПоПеремещениюФакт.КоличествоУпаковок КАК КоличествоУпаковокФакт,
			|	втПринятоПоПеремещению.КоличествоФакт - втСтрокиДокументаОтгузки.Количество КАК КоличествоПлан,
			|	втПринятоПоПеремещению.КоличествоУпаковок - втСтрокиДокументаОтгузки.КоличествоУпаковок КАК КоличествоУпаковокПлан,
			|	втСтрокиДокументаОтгузки.Коэффициент КАК Коэффициент,
			|	ВЫБОР
	...
	&НаСервере
	Процедура ПерейтиНаСтраницуНаСервере(Страница = Неопределено)
	...
	Процедура ПослатьПисьмоВСлужбуБезопасности()
		...
		// зафиксируем в журнале регистрации
			ЗаписьЖурналаРегистрации("АРМПриемка.ОшибкаОтправкиПочты", УровеньЖурналаРегистрации.Ошибка, , , "Не найден пользователь "
			...
			// только в этом случае создадим  письмо
				...
				Попытка
					Заголовок = Строка(ТекущаяДата())
						+ " [АРМПриемка]: водитель не передал упаковочный лист приемщику";
					...
					Исключение
						УспехОтправки = Ложь;
						Почта.Отключиться();
						ЗаписьЖурналаРегистрации("АРМПриемка.ОшибкаОтправкиПочты", УровеньЖурналаРегистрации.Ошибка, , ,
												 "Ошибка Отправки Письма ( ООП )" + ОшибочныеПолучатели);
					КонецПопытки;
				...
				КонецПопытки;
			Иначе
				ЗаписьЖурналаРегистрации("АРМприемка.ОшибкаОтправкиПочты", УровеньЖурналаРегистрации.Ошибка, , , "Не получилось отправить письмо о возврате: неверные параметры сервера");
			КонецЕсли;
